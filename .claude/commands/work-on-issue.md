...
スタンドアロン実行と、`/parallel-work` からサブエージェントとして呼び出される両モードに対応します。
...
1. イシューの「やること」に従い、順次実装する
2. 各タスクを完了したら、チェックリストの `[ ]` を `[x]` に更新する意識で進める
3. 実装中に設計判断が必要な場合は、簡潔にコメントとして記録する
