...
`/design` の Step 1.7 でバリエーションモード（単一 / 3案）を選択可能。複数バリエーション選択時は A/B/C の3パターンを生成し、顧客に選択肢を提示する。

**成果物サマリの提示:**
- 生成した画面数（バリエーション時は × バリエーション数）
- 適用したカラーテーマ（バリエーション時は比較テーブル）
- Figma ファイル URL（マークダウンリンク）
...
- 各ステップでエラーが発生した場合、エラー内容を報告し、スキップするか中止するか **必ず** AskUserQuestion で確認
