...
graph TD
    ...
    subgraph "4つの事業部（レーン）"
        ...
        subgraph Coding["⚙️ 開発部"]
            C1["/work-on-issue<br/>単体イシュー実装"]
            C2["/parallel-work<br/>並列イシュー実装"]
            C3["/creating-pr<br/>PR作成"]
            C4["/reviewing<br/>レビュー"]
        end

        subgraph Analysis["📋 企画・分析部"]
            A1["/extract-requirements<br/>要件抽出"]
            A2["/define-requirements<br/>要件定義"]
            A3["/generate-screens<br/>画面設計"]
            A4["/wireframe<br/>ワイヤーフレーム"]
        end

        subgraph QA["🧪 品質保証部"]
            Q1["/generate-test-cases<br/>テストケース生成"]
            Q2["/run-ui-test<br/>UIテスト実行"]
            Q3["/test-report<br/>テストレポート"]
        end

        subgraph Design["🎨 デザイン部"]
            D1["/sync-design-system<br/>デザインシステム同期"]
            D2["/design<br/>デザイン作成"]
            D3["/design-review<br/>デザインレビュー"]
        end
    end
    ...
    subgraph Support["📚 管理・支援機能（スタッフ部門）"]
        direction LR
        S1["ルール (.claude/rules/)<br/>5つの行動規範"]
        S2["テンプレート (knowledge/templates/)<br/>7つの定型書式"]
        S3["チェックリスト (knowledge/review/)<br/>2つのレビュー基準"]
        S4["フック (.claude/hooks/)<br/>2つの安全装置"]
    end

    TL -.->|参照| Support

    subgraph Temp["👷 動的チームメンバー<br/>（並列実行時のみ生成）"]
        M1["メンバー #1<br/>worktree A"]
        M2["メンバー #2<br/>worktree B"]
        M3["メンバー #N<br/>worktree N"]
    end
    ...
    TL -.->|MCP連携| Gmail
    Design -.->|MCP連携| Figma
    Analysis -.->|MCP連携| GDocs

    subgraph Outputs["📦 成果物倉庫<br/>docs/outputs/{案件slug}/"]
        O1["requirements.md"]
        O2["screens/"]
        O3["wireframes/"]
        O4["estimate-doc.md"]
