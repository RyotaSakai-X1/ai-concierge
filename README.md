...
**チャットで指示するだけで、要件定義・デザイン・実装・PR作成まで一気通貫で自動化する AI エージェント基盤。**
PM・デザイナー・エンジニアの境界を取り払い、1人で数日分の工程をチャット数往復で完結できます。

| 従来 | ai-concierge |
|------|-------------|
| 要件定義書を手作業で書く | `/define-requirements` で案件情報から自動生成 |
| 見積もりをスプシで手計算 | `/estimating` で工数自動算出・スプシ書き戻し |
| デザイナーにワイヤーフレームを依頼して待つ | `/wireframe` で要件定義から FigJam に自動生成 |
| Figma の画面設計を人手でレビュー | `/design-review` でデザインレビューを即時実行 |
| イシューを1つずつ実装 | `/parallel-work` で3件同時に並列実装・PR作成 |
| レビューを人に依頼して待つ | `/reviewing` でセルフレビュー即時実行 |
| 議事録からタスクを手動起票 | `/create-issues-from-meeting` で一括 Issue 化 |
