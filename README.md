...
| 今日の予定を確認したい         | `/today`         | `/today`                                  | Google Calendar から今日の予定を取得し、関連情報があれば添えて報告します |
...
**4. OAuth クライアント ID を作成**（各ユーザーがブラウザでログインして認証する方式です。管理者が一括管理したい場合はサービスアカウント方式も選べます）

1. 「APIとサービス」→「認証情報」→「認証情報を作成」→「OAuth クライアント ID」
2. アプリケーションの種類: **デスクトップアプリケーション**
3. 名前は何でも OK（例: `Claude Code 連携`）
4. 作成後に表示される **Client ID** と **Client Secret** をコピー
